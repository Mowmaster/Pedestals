...
public class ItemUpgradeBlastFurnace extends ItemUpgradeBase
{
    public ItemUpgradeBlastFurnace(Properties p_41383_) {
    ...
    @Nullable
    protected BlastingRecipe getNormalRecipe(BasePedestalBlockEntity pedestal, ItemStack stackIn) {
    ...
    protected Collection<ItemStack> getNormalResults(BlastingRecipe recipe) {
        return (recipe == null)?(Arrays.asList(ItemStack.EMPTY)):(Collections.singleton(recipe.getResultItem()));
    }

    protected int getCookTime(BlastingRecipe recipe) {
        return (recipe == null)?(0):(recipe.getCookingTime());
    }

    protected int getExperienceResult(BlastingRecipe recipe) {
    ...
    {
        BlastingRecipe recipe = getNormalRecipe(pedestal, input);
        if(recipe != null)
        {
                                ...
                                if(hasCachedRecipe(coin))
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        if(pedestal.removeEnergy(cost,true)>=cost)
                                        {
                                            ...
                                            if(pedestal.canSpawnParticles()) MowLibPacketHandler.sendToNearby(pedestal.getLevel(),pedestal.getPos(),new MowLibPacketParticles(MowLibPacketParticles.EffectType.ANY_COLOR_CENTERED,pedestalPos.getX(),pedestalPos.getY()+0.75D,pedestalPos.getZ(),50,50,75));
                                        }
                                    }
                                }
                                else
