...
public class ItemUpgradeSmelter extends ItemUpgradeBase
{
    public ItemUpgradeSmelter(Properties p_41383_) {
    ...
    @Nullable
    protected SmeltingRecipe getNormalRecipe(BasePedestalBlockEntity pedestal, ItemStack stackIn) {
        ...
        Optional<SmeltingRecipe> optional = recipeManager.getRecipeFor(RecipeType.SMELTING, cont, level);
    ...
    protected Collection<ItemStack> getNormalResults(SmeltingRecipe recipe) {
        return (recipe == null)?(Arrays.asList(ItemStack.EMPTY)):(Collections.singleton(recipe.getResultItem()));
    }

    protected int getCookTime(SmeltingRecipe recipe) {
        return (recipe == null)?(0):(recipe.getCookingTime());
    }

    protected int getExperienceResult(SmeltingRecipe recipe) {
    ...
    {
        SmeltingRecipe recipe = getNormalRecipe(pedestal, input);
        if(recipe != null)
        {
