...
public class ParticleBeam extends ParticleBase {
    int delay;

    public ParticleBeam(BlockPos toPos, BlockPos fromPos, int delay, ClientWorld world){
    ...
    public static LinkedList<ParticleElem> beam(BlockPos toThisBlock, BlockPos fromThisBlock){
        ...
        while ((d4) < d3)
        {
            ...
            elemArrayList.add(new ParticleElem(new RedstoneParticleData(0.0f,1.0f,0.0f,1.0f),x1 + d0 * d4, y1 + d1 * d4, z1 + d2 * d4, 0.0D, 0.0D, 0.0D));
        }
