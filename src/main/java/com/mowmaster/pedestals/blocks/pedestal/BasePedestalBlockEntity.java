...
public class BasePedestalBlockEntity extends MowLibBaseBlockEntity
{
    ...
    public void update()
    {
        BlockState state = Objects.requireNonNull(level).getBlockState(getPos());
        ...
        return new ItemStackHandler(27) {
            ...
            */
            ...
            public ItemStack extractItem(int slot, int amount, boolean simulate) {
                ...
                return super.extractItem((slot>getSlots())?(0):(slot), Math.min(amount, !filter.getFilterDirection().extract() ? amount : filter.canAcceptCountItems(getPedestal(),getFilterInPedestal(),  getStackInSlot(slot>getSlots() ? 0 : slot).getMaxStackSize(), getSlotSizeLimit(),getStackInSlot(slot>getSlots() ? 0 : slot))), simulate);
            }
        };
    }
    ...
    public void setFakePlayer()
    {
        ...
        {
            Objects.requireNonNull(pedestalPlayer.get()).setPos(getPos().getX(),getPos().getY(),getPos().getZ());
            Objects.requireNonNull(pedestalPlayer.get()).setRespawnPosition(Objects.requireNonNull(pedestalPlayer.get()).getRespawnDimension(), getPos(),0F,false,false);
        }
    }
    ...
    public void actionOnRemovedFromPedestal() {
        // 0 = Dropped
        // 1 = Removed
    ...
    public boolean isPedestalBlockPowered(BasePedestalBlockEntity pedestal)
    {
        if(pedestal.hasRedstone())
        {
            ...
            return !Objects.requireNonNull(this.getLevel()).hasNeighborSignal(pedestal.getBlockPos()) || (pedestal.getRedstonePower() < pedestal.getRedstonePowerNeeded());
        }
    ...
    private boolean transferActionImpl(List<BlockPos> view, int startIndex) {
        ...
        for (Iterator<BlockPos> it = view.iterator(); it.hasNext(); ++numScanned) {
            ...
            if(Objects.requireNonNull(level).getBlockEntity(posReceiver) instanceof BasePedestalBlockEntity pedestal) {
