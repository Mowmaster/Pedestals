...
{
    public ItemUpgradeShearer(Properties builder) {super(builder.tab(PEDESTALS_TAB));}
        ...
        if(baa.size() > 0)
        {
            for(LivingEntity baaaaaa : baa)
            {
                if(baaaaaa instanceof IForgeShearable)
                {
                    ...
                    if (baabaa.isShearable(new ItemStack(Items.SHEARS),world,new BlockPos(baaaaaa.getBlockPositionVec())))
                    {
                        if(getStackInPedestal(world,pedestalPos).equals(ItemStack.EMPTY))
                        {
                            Random rando = new Random();
                            int i = 1 + rando.nextInt(3);
                            List<ItemStack> drops = baabaa.onSheared(null,new ItemStack(Items.SHEARS),world,new BlockPos(baaaaaa.getBlockPositionVec()),0);
                            ...
                            for (int j = 0; j < i; ++j)
                            {
                                if(drops.size()>0)
                                {
                                    for (int d=0;d<drops.size();d++)
                                    {
                                        ...
                                        {
                                            BlockPos sheerie = baaaaaa.getBlockPosition();
    ...
    public static final Item SHEARER = new ItemUpgradeShearer(new Properties().stacksTo(64).tab(PEDESTALS_TAB)).setRegistryName(new ResourceLocation(MODID, "coin/shearer"));
