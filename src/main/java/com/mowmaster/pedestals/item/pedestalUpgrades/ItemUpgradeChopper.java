...
{
    public ItemUpgradeChopper(Item.Properties builder) {super(builder.tab(PEDESTALS_TAB));}
    ...
    public boolean canMineBlock(PedestalTileEntity pedestal, BlockPos blockToMinePos, PlayerEntity player)
    {
        ...
        if(!blockToChop.isAir(blockStateToChop,world,blockToMinePos))
        {
            ...
            if((blockToChop.isIn(BlockTags.LOGS) || blockToChop.isIn(BlockTags.LEAVES)) && passesFilter(world, pedestal.getBlockPos(), blockToChop))
            {
                return true;
            }
        }
    ...
    public static final Item CHOPPER = new ItemUpgradeChopper(new Item.Properties().stacksTo(64).tab(PEDESTALS_TAB)).setRegistryName(new ResourceLocation(MODID, "coin/chopper"));
