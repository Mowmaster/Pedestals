...
{
    public ItemFilterEnchanted(Properties builder) {super(builder.group(PEDESTALS_TAB));}
    ...
    public static void handleItemColors(ColorHandlerEvent.Item event) {
        event.getItemColors().register((itemstack, tintIndex) -> {if (tintIndex == 1){return getColorFromNBT(itemstack);} else {return -1;}},ENCHANTEDFILTER);
    }

    public static final Item ENCHANTEDFILTER = new ItemFilterEnchanted(new Properties().maxStackSize(64).group(PEDESTALS_TAB)).setRegistryName(new ResourceLocation(MODID, "filter/filterenchanted"));

    @SubscribeEvent
    public static void onItemRegistryReady(RegistryEvent.Register<Item> event)
    {
        event.getRegistry().register(ENCHANTEDFILTER);
    }
}
