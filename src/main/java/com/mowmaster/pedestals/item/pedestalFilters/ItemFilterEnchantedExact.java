...
public class ItemFilterEnchantedExact extends ItemFilterBase
{
    public ItemFilterEnchantedExact(Properties builder) {super(builder.group(PEDESTALS_TAB));}
    ...
    public static void handleItemColors(ColorHandlerEvent.Item event) {
        event.getItemColors().register((itemstack, tintIndex) -> {if (tintIndex == 1){return getColorFromNBT(itemstack);} else {return -1;}},ENCHANTEDEXACTFILTER);
    }

    public static final Item ENCHANTEDEXACTFILTER = new ItemFilterEnchantedExact(new Properties().maxStackSize(64).group(PEDESTALS_TAB)).setRegistryName(new ResourceLocation(MODID, "filter/filterenchantedexact"));

    @SubscribeEvent
    public static void onItemRegistryReady(RegistryEvent.Register<Item> event)
    {
        event.getRegistry().register(ENCHANTEDEXACTFILTER);
    }
}
