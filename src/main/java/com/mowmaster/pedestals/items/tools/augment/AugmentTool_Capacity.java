...
public class AugmentTool_Capacity extends BaseTool implements IPedestalTool
{
    ...
    @Override
    public ItemStack getMainTool() { return DeferredRegisterItems.TOOL_AUGMENTS_CAPACITY.get().getDefaultInstance(); }

    @Override
    public ItemStack getSwappedTool() { return DeferredRegisterItems.TOOL_AUGMENTS_STORAGE.get().getDefaultInstance(); }
    ...
    public void getPedestalDetail(BasePedestalBlockEntity pedestal, Player player) {
        ...
        if(pedestal.hasCapacity())
        {
            ItemStack getCurrentAugments = pedestal.currentCapacityAugments();
            int currentInsertAmount = getCurrentAugments.getCount();
            int maxInsertAmount = AugmentTieredCapacity.getAllowedInsertAmount(getCurrentAugments);
        ...
        {
            MowLibMessageUtils.messagePlayerChat(player,ChatFormatting.DARK_AQUA,MODID + ".tool.augment.capacity_not");
        }
    }
}
